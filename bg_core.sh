...
if [[ ! "${_importedLibraries@a}" =~ A ]] || [ ! "${_importedLibraries["lib:bg_coreImport.sh"]}" ]; then
	...
	# by non-root users. The important thing is to prevent, on a production host, a non-priviledged user's script (that they can modify)
	# into tricking this sourced library into falling into the last, default case where it would set bgProductionMode='development'.
	# When other system library scripts (which are installed in folders that only priviledged users can modify) are sourced, they use the
	# variables set by this block to determine if insecure development features can be used. For example, in development mode, a script
	# can invoke a debugger UI which is not allowed in production mode..
	...
	if [ "$1" != "--queryMode" ]; then
		...
		function _coreFindInclude()
		{
			...
			if [ ! "$bg_coreImport_path" ]; then
				if [[ "$1" =~ ^--pre(inst|rm)$ ]]; then
					earlyTrace "BGINSTALL: bg_core libraries not yet installed on host (but bg_core.sh is installed by bg_debugCntr) 'source bg_core.sh $*'"
					exit 0
				fi
